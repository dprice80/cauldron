...
def fun_build_base_image(use_cache: bool = True):
    ...
    sub.run(f"""
            ...
            docker build{cache_flag} -t {base_image} -f {base_dockerfile} --build-arg PROJECT_NAME={config['CAUL_PROJECT_NAME']} . | tee docker_base_build.log
    ...
    if use_docker:
        ...
        run_command = f"""
            ...
            docker build -t {IMAGE_NAME} -f {dockerfile} --build-arg PROJECT_NAME={project_name} --build-arg \
                IMAGE_TAG={VERSION_TAG} --build-arg USER_PREFIX={user_prefix}{use_cache_arg} .
            """
